...
void main() {
    float h = textureTile(elevationSampler, uvz.xy).x;
    if (h <= 0.0) {
        discard;
    }
    vec3 n = vec3(textureTile(fragmentNormalSampler, uvz.xy).xy * 2.0 - 1.0, 0.0);
    n.z = sqrt(max(0.0, 1.0 - dot(n.xy, n.xy)));
    vec3 l = getWorldSunDir();
    l *= (l.z > 0.0 ? 1.0 : 0.0);
    float v = uvz.z < 0.5 ? 0.6 * max(l.z, 0.0) : max(dot(n, l), 0.0);
    data = vec4(vec3(v), 1.0);
}
